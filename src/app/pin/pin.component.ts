...
export class PinComponent {
  ...
  @Input() isSecretMode = false; // isSecretMode = true is secret mode
  @ViewChildren('input') inputsList!: QueryList<ElementRef>;

  @Input() regexValidate = /^[0-9]+$/; // allow change  regex for check input
  @Output() getPin = new EventEmitter<string>();
  ...
  private pinChanges(): void {
    setTimeout(() => {
      ...
      if (code.length >= this.codeLength) {
        this.pinResult = code;
        this.getPin.emit(this.pinResult);
      }
    }, 50);
  }
